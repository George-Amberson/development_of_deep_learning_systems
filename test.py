...
parser.add_argument('--label_file', default='./demo/label/label.png', type=str, help='Label file')
...
with torch.no_grad():
    for file_ in tqdm(files):
        ...
        if task == 'Gaussian_Gray_Denoising':
            ...
            output = Image.open(os.path.join(out_dir, f+'.png')).convert('RGB')
            output = default_toTensor(output)
            psnr = -10*torch.log10(torch.mean((label-output)**2))
            print("\n"+70*"=")
            print(f"\n PSNR = {psnr} \n")
            print(70*"=")
            if psnr > 40:
